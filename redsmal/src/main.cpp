...
double RampPower=28; //not yet used
...
void autonRedSmall(){ //still need to add the intake and stacking
  ...
  moveForward(-0.1,-1,0,95);
  moveForward(0.4,1,0,35);
 // wait(0.1, sec); //wait for the cube to all get sucked up
  // move backward while moving to the left
...
void onecubeBlueBig(){
  moveForward(0.6, 0, 1, 100);
  moveForward(0.5, 0, -1, 100);
}
...
void autonBlueSmall(){ //still need to add the intake and stacking MAINONE
  intakeCubes(80);
...
void usercontrol(void){
  bool ramping=false;
    ...
    //     Controller1.rumble(". . .");
    //   }else{
    //     Brain.Screen.printAt(30,60, "%d, %d, %d, %d", LeftMotor.temperature(pct), RightMotor.temperature(pct), intakeLeft.temperature(pct), intakeRight.temperature(pct));
    //   }
    // }
